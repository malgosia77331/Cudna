...
matrixMulCUDAver1(float *C, float *A, float *B, int width)
{
    int tx = threadIdx.x;
    int ty = threadIdx.y;
...
//wersja trzecia
template <int BLOCK_SIZE> __global__ void
matrixMulCUDA3(float *C, float *A, float *B, int wA, int wB)
{
    // Block index
    int bx = blockIdx.x;
    int by = blockIdx.y;
...
//wersja czwarta
template <int BLOCK_SIZE> __global__ void
matrixMulCUDA4(float *C, float *A, float *B, int wA, int wB)
{
    int bx = blockIdx.x;
    int by = blockIdx.y;
	int tx = threadIdx.x;
	...
	__shared__ float As[BLOCK_SIZE][BLOCK_SIZE];
	__shared__ float Bs[BLOCK_SIZE][BLOCK_SIZE];
	float As2 = A[aBegin + wA * ty + tx];
	float Bs2 = B[bBegin + wB * ty + tx];
	
	for (int a = aBegin+aStep, b = bBegin+bStep; a <= aEnd+aStep; a += aStep, b += bStep)
    {
        As[ty][tx] = As2;
        Bs[ty][tx] = Bs2;
...
int matrixMultiply(int argc, char **argv, int block_size, dim3 &dimsA, dim3 &dimsB)
{
    ...
    if (block_size == 16)
    {
        //matrixMulCUDA<16><<< grid, threads >>>(d_C, d_A, d_B, dimsA.x); //dla ver1 i ver2
		matrixMulCUDA<16><<< grid, threads >>>(d_C, d_A, d_B, dimsA.x, dimsB.x); //dla ver3 i ver4
    }
    else
    {
